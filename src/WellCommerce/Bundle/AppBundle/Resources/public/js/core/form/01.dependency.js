...
var GFormDependency = function(sType, sFieldSource, mCondition, mArgument) {
  ...
  gThis._InitializeEvents = function() {
    ...
    if (gThis.m_sType === GFormDependency.EXCHANGE_OPTIONS) {
      gField.TriggerChangeHandler();
    }
  };
  ...
  gThis._FindFieldInCurrentRepetition = function(gCurrentField, gDependentField) {
    if ((gCurrentField.m_gParent instanceof GFormRepetition) &&
        (gCurrentField.m_gParent.m_gParent === gDependentField.m_gParent.m_gParent)) {
  ...
  gThis.EvaluateExchangeOptions = function(eEvent) {
    ...
    GF_Ajax_Request(Routing.generate(gThis.m_fSource), {
      value: eEvent.data.gNode.GetValue(),
    }, function(oData) {
      gThis.m_gForm.GetField(gThis.m_sFieldTarget).ExchangeOptions(oData);
    });
  };
